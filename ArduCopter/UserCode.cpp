	...
	*/ 
	if(g2.user_parameters.getAcrtEnable()){
		...
		if(g2.user_parameters.getAcrtDebug()){
			gcs().send_text(MAV_SEVERITY_INFO, "ACTR Activated");
		}
		...
		if(g2.user_parameters.getAcrtDebug()){
			gcs().send_text(MAV_SEVERITY_INFO, "ACTR motor position %f and array idx %u", motorPosRads, motorPosTabIdx);
		}
		...
		if (pitchCTRL > 0.0 || pitchCTRL < 0.0) {
			if (rollCTRL > 0.0 || rollCTRL < 0.0){
				angleDep = atan2(rollCTRL, pitchCTRL);
				if (angleDep < 0){
		...
		//gcs:send_text(MAV_SEVERITY.INFO, " PRM_SRVFCT:get() " .. PRM_SRVFCT:get() .. " SRV_MIN:get() " .. SRV_MIN:get() .. " SRV_MAX:get() " .. SRV_MAX:get() .. " absolutePicth " .. absolutePicth .. " radStepForDep " .. radStepForDepMax .. " minCmdVal " .. minCmdVal .. " maxCmdVal " .. maxCmdVal .. " outputMinCmdVal " .. outputMinCmdVal .. " outputMaxCmdVal " .. outputMaxCmdVal .. " thrustOscillant " .. thrustOscillant)
	    //gcs:send_text(MAV_SEVERITY.INFO, "Thrust oscillant " .. thrustOscillant)
	  ...
	  //gcs:send_text(MAV_SEVERITY.INFO, " CALCULS " .. tostring(timer))
